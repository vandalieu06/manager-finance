...
type Tag struct {
	...
	UserID    *uint  `gorm:"index"` // MEJORA 5: Tags privados por usuario o nulo si son globales
	CreatedAt time.Time
	// Relaciones
	Users        []User        `gorm:"many2many:user_tags;"`
	Categories   []Category    `gorm:"many2many:category_tags;"`
	Transactions []Transaction `gorm:"many2many:transaction_tags;"` // MEJORA 2
}
...
type Transaction struct {
	...
	Type        string    `gorm:"type:varchar(20);not null;check:type IN ('income', 'expense')"`
	Amount      Money     `gorm:"not null;default:0"`         // MEJORA 1: Usando enteros
	Currency    string    `gorm:"type:char(3);default:'EUR'"` // MEJORA 3: ISO Code (EUR, USD, MXN...)
...
type Product struct {
	...
	Price         Money  `gorm:"not null"` // MEJORA 1: Precio unitario en enteros
	Quantity      int    `gorm:"not null;default:1"`
	ShopName      string `gorm:"type:varchar(255)"`
	CreatedAt     time.Time
	UpdatedAt     time.Time
	DeletedAt     gorm.DeletedAt `gorm:"index"` // Añadido para consistencia con el padre
}

// --- MEJORA 4: Hook para Borrado Lógico en Cascada ---

// AfterDelete asegura que si borramos una transacción, sus facturas y productos
// se marquen también como borrados lógicamente.
